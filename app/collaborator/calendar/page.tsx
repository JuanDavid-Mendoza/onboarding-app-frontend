...
function ColaboradorCalendarioContent() {
  const { user } = useAuth()
  const { getUserOnboardings } = useData()
  const { toast } = useToast()
  const [selectedOnboarding, setSelectedOnboarding] = useState<(Onboarding & { state: number }) | null>(null)
    ...
    <CollaboratorLayout>
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Visualiza tus onboardings programados</p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 pt-4 border-t border-border">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`font-medium ${selectedOnboarding.state == 1 ? "text-green-600" : "text-yellow-600"}`}>
                    {selectedOnboarding.state == 1 ? "Completado" : "Pendiente"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
